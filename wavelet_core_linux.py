...
class ScatteringEqualNet_Batch_Good_Cuda(nn.Module):
    def __init__(self):
        "Defines the parameters of the model."
        super(ScatteringEqualNet_Batch_Good_Cuda, self).__init__()
        
        self.scattering = nn.Sequential(OrderedDict([
                    ('conv1', nn.Conv2d(3, 81, kernel_size=5, padding=2)),
                    ('bn1', nn.BatchNorm2d(81)),
                    ('pool1', nn.MaxPool2d(kernel_size=(2, 2))),
                    ('relu1', nn.ReLU()),
                    
                    ('conv2', nn.Conv2d(81, 243, kernel_size=5, padding=2)),
                    ('pool2', nn.MaxPool2d(kernel_size=(2, 2))),
                    ('relu2', nn.ReLU()),
                ]))             
...
class ScatteringEqualNet_Batch_Good_Cuda_hiden(nn.Module):
    def __init__(self):
        "Defines the parameters of the model."
        super(ScatteringEqualNet_Batch_Good_Cuda_hiden, self).__init__()
